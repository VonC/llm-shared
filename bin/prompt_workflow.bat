...
REM Picks the most recent venv whose name contains "llm-shared" (newest first,
...
REM so a nested cmd or a foreign PATH cannot resolve the wrong interpreter. The
REM trailing "*" matters: a worktree venv is named python_3.13.9_llm-shared_main
REM (note the _main suffix), so the glob must allow characters after
REM "llm-shared" while still skipping a copilot-shared venv.
