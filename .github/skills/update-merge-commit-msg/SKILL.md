...
For that, call #file:./git-extract-merge-docs.sh making sure to use bash -c "%COPILOT_SHARED_DIR_UNIX%/.github/skills/update-merge-commit-msg/git-extract-merge-docs.sh". No parameter is needed, can be called from anywhere, but you need to resolve the full path of the script.
