...
- if it is not yet implemented, do not write code, but explain what is missing, and record it as a `### Missing work for Step x` sub-section of the validation plan: one bullet per missing element (code, test, wiring, or a file over the line budget), each concrete enough to be implemented without re-deriving this analysis. That section is the work list the [`implement-missing-step.md`](implement-missing-step.md) instruction reads, so a "No, it is not implemented" status without a `Missing work for Step x` section is an incomplete check.
