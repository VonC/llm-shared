...
- if it is not yet implemented, do not write code, but explain what is missing, and record it as a `### Missing work for Step x` sub-section of the validation plan, placed right after `### What was implemented for Step x` as the template orders it: one bullet per missing element (code, test, wiring, or a file over the line budget), each concrete enough to be implemented without re-deriving this analysis. Any status other than a full "Yes, it is implemented" counts as not implemented for this rule: "No, it is not fully implemented", "partially implemented", "mostly implemented", "implemented except ..." all require the section. Gaps already described in the Architecture, Performance, Unit test coverage, or Feature integrity sub-sections must be repeated here as bullets: noting them there does not replace this section. That section is the single work list the [`implement-missing-step.md`](implement-missing-step.md) instruction reads, so a not-fully-implemented status without a `Missing work for Step x` section is an incomplete check.
...
Check your validation plan update: re-read the status sentence you wrote under `### Analysis of Step x implementation state`. If it is anything other than a full "Yes, it is implemented", search the updated document for the `### Missing work for Step x` heading. If that heading is absent, the check is not finished: add the section with the gathered missing-element bullets before ending your answer.
