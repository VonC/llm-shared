...
### Verify a project without a pytest suite

Exit `9` means the project root has no pytest suite: no `pyproject.toml`, `pytest.ini` or `conftest.py`, and no pytest section in `setup.cfg` or `tox.ini`. Groundhog then has no test step to run. The walk still ran check.bat first, when the project has one, and it passed, so do not run check.bat again.

Validate the tests with the project's own commands instead: the ones the plan, the validation plan, or the project instructions name for this step, such as a verify script. Run each once, fix what fails, and re-run it until it passes. After a code fix, run `ghog day` again first, so check.bat re-proves the change and the walk ends on exit 9 again.

Exit 9 is not a setup error. Never install pytest, create a venv, or add a `pyproject.toml` to turn it into a full walk.
...
When the step is implemented and the `ghog day` walk reports the objective (`exit=0`), or exit 9 followed by a green run of the project's own test commands, hand the cycle on to the implementation check, with no menu. From the project root, run:
