...
For that, call [`git-extract-merge-docs.sh`](./git-extract-merge-docs.sh) making sure to use bash -c "%COPILOT_SHARED_DIR_UNIX%/.claude/skills/update-merge-commit-msg/git-extract-merge-docs.sh". No parameter is needed, can be called from anywhere, but you need to resolve the full path of the script.

If the script is successful, it creates `a.docs` with the extracted docs from the merged branch: analyse its content to write the `a.commit` message with a conventional commit message following the "Commit message" section of [`group-commits-msg/SKILL.md`](../group-commits-msg/SKILL.md) and the template in [`group-commits-msg/TEMPLATE.md`](../group-commits-msg/TEMPLATE.md).  
