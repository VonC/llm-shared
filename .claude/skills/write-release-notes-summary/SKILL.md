---
name: write-release-notes-summary
description: 'Write a concise release notes summary draft from a list of conventional commit subjects (and any `docs/plan.xxx.md` plan files in context), then propose three witty title/subtitle pairs. Use when the user asks for "release notes", "release summary", or "write release notes summary".'
user-invocable: true
argument-hint: 'Optionally provide the version vX.Y.Z and topic, for example "v9.3.0 sentinels". Otherwise the skill works from the conventional commit subjects already in context.'
---
...
- Follow the markdown list rules from
  [`markdown.instructions.md`](../../markdown.instructions.md): one
  space after the list marker, an empty line before the first item and
  after the last item, two-space indents for sub-items.
- Do not use words listed in
  [`blacklist.md`](../../blacklist.md). Replace them with simpler,
  direct language.
