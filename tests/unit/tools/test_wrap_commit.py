...
``type(scope):`` line opener.

Fix: split for the repo line budget -- the word-rule classes moved to
``test_wrap_commit_words.py``, the reflow classes to
``test_wrap_commit_reflow.py``, the driver plus CLI classes to
``test_wrap_commit_cli.py``, and the ``tools.wrap_commit_wraplist``
collection classes (search order, reader, collector) to
``test_wrap_commit_wraplist.py``; the targets now live in the split
``tools.wrap_commit_backticks`` module, reached directly by these tests.
"""
