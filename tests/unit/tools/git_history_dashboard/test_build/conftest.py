"""Shared fixtures for the git-history dashboard build tests.

Step 3 (v0.8.0): ``write_dashboard`` converts the analysis markdown through the
``uv``-backed ``convert_markdown`` seam. This autouse fixture stubs that seam so
every build test stays fast and needs neither ``uv`` nor the ``markdown``
package; the real seam is covered directly in ``test_analysis``.
"""
