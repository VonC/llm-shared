...
- The analysis is hand-written inside `template.html` and pdfsplitter-specific; there is no slot, no generated-figures file, and no markdown conversion.
...
- `tools/git_history_dashboard/template.html` (existing, to be updated) -- add `__ANALYSIS__` and `__TITLE__` slots; drop the hardcoded pdfsplitter strings and the inline observations content.
...
- `test_render_tdd.py` (update): `render` fills `__ANALYSIS__` and `__TITLE__`; the rendered HTML carries no pdfsplitter string; a single project yields its name as the title, several yield the project-count label.
...
- `ghog day` reports the objective (`exit=0`).
- `rg -n "pdfsplitter" tools/git_history_dashboard/template.html` returns nothing.
...
- `test_report_acceptance_tdd.py` (larger than unit; builds throwaway git repos under `tmp_path` with real `git`): a two-repo `--out-dir` run writes one `data.json` whose payload has `projects` (both), `by_project` (summing to the top-level series), and `by_author`; the `dashboard.html` has the filled `__ANALYSIS__` and `__TITLE__` slots and no pdfsplitter string; `analysis.notes.md` survives a second run unchanged while `analysis.generated.md` refreshes; `--no-open` writes the report without calling the opener.
