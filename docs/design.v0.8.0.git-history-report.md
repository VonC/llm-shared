...
**The template is pdfsplitter-specific in more than the observations**: the `<title>`, the header `<h1>`, the `sr-only` description, the hand-written `observations` section, and the footer command are all hardcoded for pdfsplitter. Making the report project-neutral covers all of these, not only the observations block.
...
### Parameterizing the pdfsplitter strings

The hardcoded pdfsplitter strings (the `<title>`, the header `<h1>`, the
