...
## Implementation decisions for v0.12.0 docs_layout_hardening

All six implementation questions are settled. The four steps can proceed in
order, with their individual validation gates and the existing design unchanged.

| Question | Decision and reason | Integrated in | Rejected alternatives |
| --- | --- | --- | --- |
| Q01 | Extract the cohesive lookup block into `prompt_workflow_document_lookup.py`, preserving facade exports. This creates headroom in the responsibility being extended and keeps its dependency on models one-way. | Confirmed code and test facts; Step 1 implementation. | Extracting collection parsing would leave the growing lookup responsibility mixed with topic and marker work. |
| Q02 | Retain four numbered steps, each with a successful groundhog walk. Separate extraction, recognition and selection checkpoints make failures attributable to one change. | Plan goal and scope; shared execution checklist; Steps 1 through 4. | Combining extraction and eligibility would mix relocation regressions with changed recognition behavior. |
| Q03 | Use bounded filename properties and explicit filesystem scenarios. Generated identity cases suit the filename grammar; named add/rename/remove cases directly cover freshness. | Step 2 implementation and addendums. | Stateful filesystem property tests add lifecycle and shrinking complexity for few relevant states. |
| Q04 | Scope IO guards and injected errors to recognition or candidate discovery. This proves the accepted boundaries while allowing existing downstream body and mtime reads. | File-based IO cost clarification; complexity and timing clarification; Steps 2 and 3 implementation. | Broad CLI filesystem guards can reject permitted reads; real permission failures vary with platform and privileges. |
| Q05 | Use small real temporary Git repositories for representative CLI acceptance, isolating host delivery only. This verifies branch-derived discovery and error reachability through real selection and routing. | Step 4 implementation. | Stubbing Git in every new acceptance case would leave the discovery-to-selection boundary unproven. |
| Q06 | Parameterize roles against selection outcomes, then add focused layout, version and subtopic cases. Map the evidence to all sixteen design acceptance rows. | Step 3 implementation; Step 4 completion. | A full Cartesian product would repeat equivalent behavior and increase fixture cost and maintenance. |
