...
`llm-shared\tools\git_history_dashboard\build.py` currently target my-project. I need you to include that as part of a skill able to generate a git history report for any project, as well as update its associated comment.

## Existing content

- the shared ghd tool, `llm-shared\tools\git_history_dashboard\build.py`, targeting my-project.
- the template `llm-shared\tools\git_history_dashboard\template.html`, targeting my-project, with a LLM-written comment/observations block.

Outputs:

- `my-project\docs\git_history_dashboard\dashboard.html` and `my-project\docs\git_history_dashboard\data.json` — now 3,416 commits (was 3,152), range 26 Jun 2024 → 22 Jun 2026, over 24 months. Active days 302/727 (42%), peak week 161 (week of 9 Mar 2026), peak day 43 (12 Mar 2026). Metrics/lead auto-filled from placeholders.
