...
- `template.html` holds the static HTML shell, the placeholder tokens that `render` fills (`__DATA__`, `__TOTAL_COMMITS__`, `__START__`, and the rest), and a hand-written observations block that `build.py` never generates. That block currently describes pdfsplitter's numbers.
