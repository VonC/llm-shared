...
- `status.py`: the Q32 run lifecycle — the `a.ghog.status` read/write mechanics, the pid liveness probe, the read-only status reporter, the live-run refusal, the running/done bracket around every run, and the detached day walk (survivor spawn, senv preamble hand-off, start handshake).
