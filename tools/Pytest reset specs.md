...
| (none) | `ghog day` | walk the chain: check, then `affected --no-cov`, then `full`, stopping at the first non-green step (Q22) |
...
- Exit codes (Q12): `0` objective met, `2` test failures, `3` coverage gap, `4` suite crash, `5` environment or setup error. Callers branch on the code, never on the report wording.
